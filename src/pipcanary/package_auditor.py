...
class VersionInfo:
    ...
    def from_json(cls, data: Dict[str, Any]) -> "VersionInfo":
        info = data["info"]
        package_version = PackageVersion.from_json(info)
        vulnerabilities = data["vulnerabilities"]
        urls = data["urls"]
        hashes = {}
        for url in urls:
            package_type = url.get("packagetype")
            if package_type in ["bdist_wheel", "sdist"]:
                file_name = url["filename"]
                try:
                    hashes[file_name] = url["digests"]["sha256"]
                except KeyError:
                    logger.warning(
                        f"No sha256 hash found for file {file_name} in package {str(package_version)}"
                    )
...
class AuditReport:
    ...
    def locked_requirements(self) -> List[str]:
        lines = []
        for info in self.versions_info:
            lines.append(
                f"{info.package_version.name}=={info.package_version.version} \\\n"
            )
            for n, hash in enumerate(info.hashes.values()):
                line = f"  --hash=sha256:{hash}"
                if n + 1 < len(info.hashes):
                    line += " \\\n"
                else:
                    line += "\n"
                lines.append(line)
    ...
    def write_locked_requirements(self, locked_requirements_file: str):
        try:
            with open(locked_requirements_file, "w") as out:
                out.write(
                    "# Automatically generated by PipCanary at %s.\n"
                    % (datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                )
                out.write("#\n")
                out.writelines(self.locked_requirements())
        except IOError as e:
            raise RequirementsError(
                f"Failed to write locked requirements to {locked_requirements_file}: {str(e)}"
            )
    ...
    def audit(
        ...
        for version_data in version_list:
            ...
            if info.has_vulnerabilities:
                vulnerabilities = info.vulnerabilities(selection.ignore_vulns)
                if vulnerabilities:
        ...
        return AuditReport(
            ...
            too_recent_packages=too_recent_packages,
            ignored_vulns=list(set(ignored_vulns)),
        )
